# %%
from coordinate_converter import (coordinate2point, coordinates2segment,
                                  coordinates2segments, point2coordinate,
                                  segment2coordinates, segments2coordinates)
...
def detect_walls_with_stuff(stuffs, walls, riser_projections) -> List[Tuple[Segment, bool, bool]]:
    walls_with_stuff = []
    ...
    for wall in walls:
        ...
        wall_info = Wall(wall=wall, has_stuff=flag_wall_with_stuff, has_toilet=flag_is_toilet, stuff_point=stuff_in_wall, with_riser=is_stuff_with_riser)
        walls_with_stuff.append(wall_info)
    return walls_with_stuff

def is_wall_nighbour(wall_src, wall_dist, threshold = 90):
    if min([l1_distance(wall_src.start, wall_dist.start),
        l1_distance(wall_src.start, wall_dist.end),
        l1_distance(wall_src.end, wall_dist.start),
        l1_distance(wall_src.end, wall_dist.end)]) <= threshold:
        return True
    else:
        return False
...
def get_neighbour_wall(wall_src, wall_dist, threshold = 90):
    if min([l1_distance(wall_src.start, wall_dist.start),
        l1_distance(wall_src.start, wall_dist.end)])  <= threshold:
        return wall_src.start
    elif min([l1_distance(wall_src.end, wall_dist.start),
        l1_distance(wall_src.end, wall_dist.end)])  <= threshold:
        return wall_src.end
    else:
        return None
...
def is_pipe_projection_on_high(segment: Segment, riser_projection: Point):
    return True if (segment.start.y < riser_projection.y) or (segment.end.y < riser_projection.y) else False

def is_pipe_projection_on_right(segment: Segment, riser_projection: Point):
    return True if (segment.start.x < riser_projection.x) or (segment.end.x < riser_projection.x) else False
...
def estimate_min_height(wall, riser_projections):
    if not wall.path2wall_with_riser:
        ...
        penultimate_point_on_riser_projection = min(l1_distance(wall.path2wall_with_riser[-2].start, riser_projections),
                                                    l1_distance(wall.path2wall_with_riser[-2].end, riser_projections))
        min_height = (sum([wall_path.length for wall_path in wall.path2wall_with_riser[:-1]]) \
            + penultimate_point_on_riser_projection) * 0.02
...
def count_pipes_for_wall_with_stuff(wall, riser_projections):
    ...
    if is_parallel_Y(wall):
        if wall.start_pipe_point.y > wall.stuff_point[0].projection.y:
            start = Point(wall.start_pipe_point.x, min([point.projection.y for point in wall.stuff_point]))
            wall.stuff_point = sorted(wall.stuff_point, key=lambda x: x.projection.y)
            is_last_stuff_is_edged = False
        else:
            start = Point(wall.start_pipe_point.x, max([point.projection.y for point in wall.stuff_point]))
            wall.stuff_point = sorted(wall.stuff_point, key=lambda x: x.projection.y, reverse=True)
    else:
        if wall.start_pipe_point.x > wall.stuff_point[0].projection.x:
            start = Point(min([point.projection.x for point in wall.stuff_point]), wall.start_pipe_point.y)
            wall.stuff_point = sorted(wall.stuff_point, key=lambda x: x.projection.x)
            is_last_stuff_is_edged = False
        else:
            start = Point(max([point.projection.x for point in wall.stuff_point]), wall.start_pipe_point.y)
            wall.stuff_point = sorted(wall.stuff_point, key=lambda x: x.projection.x, reverse=True)
    if len(wall.stuff_point) == 1:
        ...
        for idx_stuff, stuff in enumerate(wall.stuff_point[:-1]):
            start_end_pipes.append(
                Pipe(
                    coordinates=Segment(start, wall.stuff_point[idx_stuff+1].projection),
                    is_toilet=stuff.is_toilet,
                    with_riser=wall.with_riser,
                    stuff=stuff))
            start = wall.stuff_point[idx_stuff+1].projection
        start_end_pipes.append(Pipe(
            coordinates=Segment(start, end),
...
def build_path(walls, riser_projections, scrennshot_name):
    ...
    for wall in walls:
        ...
        if not wall.with_riser and wall.has_stuff:
            ...
            pipes, start_height = count_pipes_for_wall_with_stuff(wall, riser_projections)
